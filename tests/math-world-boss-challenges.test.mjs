...
test("boss milestones reserve two full grade 1–2 tests without adding teaching worlds", { skip: !spiral }, () => {
  assert.deepEqual(BOSS_CHALLENGES.map(({ year, afterWorld, questionCount, gradeBand }) => ({ year, afterWorld, questionCount, gradeBand })), [
    { year: 2025, afterWorld: 10, questionCount: 24, gradeBand: "1-2" },
    { year: 2026, afterWorld: 20, questionCount: 24, gradeBand: "1-2" },
  ]);
  assert.equal(WORLD_DEFINITIONS.length, 20);
  assert.equal(WORLD_QUESTIONS.length, 480);
  assert.equal(bossAfterWorld(9), undefined);
  assert.equal(bossAfterWorld(10), bossById("boss-2025"));
  assert.equal(bossAfterWorld(20), bossById("boss-2026"));
...
test("each boss opens only after all of its preceding teaching stops", { skip: !spiral }, () => {
  ...
  assert.equal(canOpenBoss(throughWorld(10), bossById("boss-2026")), false);
});
